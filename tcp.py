...
def send_message(adu, sock,fernet):
    ...
    print("-"*40 +"connecting client to the proxy server"+"-"*40 )
    print(" Modbus commmand before encryption is " +str(fernet.decrypt(adu)))
    print(" Modbus commmand after encryption is" +str(adu))
    sock.sendall(adu)
    print("-"*40 +"connecting proxy server to client "+"-"*40 )
    r_data = sock.recv(1024)
    decryptedData  = fernet.decrypt(r_data)
    print("Encrypted Response got from server " +str(r_data))
    ...
    return parse_response_adu(response_error_adu + response_remainder, fernet.decrypt(adu))
